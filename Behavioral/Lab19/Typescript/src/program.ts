...
class Program {
    ...
    private static TestAgency(): void {
        ...
        for (const realtor of agency.Realtors) {
    ...
    private static AddTestData(agency: Agency, index: number): void {
        ...
        const a: Apartment = new Apartment(`Apartment Address ${index}`, `Description ${index}`, 50, this.GetRandomArbitrary(2000,200000), "New", 2);
        const ro: Room = new Room(`Room Address ${index}`, `Room Description ${index}`, 20, this.GetRandomArbitrary(2000,200000), "Old");
        const h: House = new House(`House Address ${index}`, `House Description ${index}`, 100, this.GetRandomArbitrary(2000,200000), "Middle aged");
        agency.AddEstate(a);
        agency.AddEstate(ro);
        agency.AddEstate(h);
