...
{    
    class Program
    {
        ...
        public static void TestAgency(){
            ...
            foreach(var a in agency.Apartments)
                visitor.Visit(a);
            foreach(var h in agency.Houses) 
                visitor.Visit(h);
            foreach(var r in agency.Rooms) 
                visitor.Visit(r);
        }
        ...
        public static void AddTestData(Agency agency, int index)
        {
            ...
            var a = new Apartment($"Apartment Address {index}", $"Description {index}", rand.NextInt64(50, 100), rand.NextInt64(2000, 200000), "New", 2);
            var ro = new Room($"Room Address {index}", $"Room Description {index}", rand.NextInt64(20, 50), rand.NextInt64(2000, 200000), "Old");
            var h = new House($"House Address {index}", $"House Description {index}", rand.NextInt64(100, 200), rand.NextInt64(2000, 200000), "Middle aged");
            agency.AddEstate(a);
            agency.AddEstate(ro);
            agency.AddEstate(h);
