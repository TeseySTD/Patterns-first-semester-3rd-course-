...
def test_agency(thread_name):
    ...
    # agency.add_apartment(builder.set_rooms_number(2).set_address(thread_name).set_price(300).set_parking().set_elevator().get_estate())
...
def add_test_data(agency, thread_name):
    ...
    # agency.add_realtor( f"Realtor {thread_name}", "123-456-7890", f"realtor{thread_name}@example.com",2)
    ...
    agency.add_apartment(EstatePool.get_instance("apartment"))
    agency.add_room(EstatePool.get_instance("room"))
    agency.add_house(EstatePool.get_instance("house"))
